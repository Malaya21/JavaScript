const string1 = "malaya";
const string3 = new String("malaya");
// console.log(string1)
// console.log(typeof string1);
// console.log(string3);
// console.log(typeof string3);

// console.log(string3[0]);
// console.log(string3[1]);
// console.log(string3[2]);
// console.log(string3[3]);
// console.log(string3[4]);

// console.log(string1.charAt(1));
// console.log(string3.charAt(1));
// console.log(string3.indexOf("l"));

// console.log(string1.toUpperCase());
// console.log(string1.toLowerCase());

// const newStr = string1.replace("Malaya","Tanmaya");//It replace the charecters and returns the new string but it can not modify the original string
// console.log(newStr);
console.log(string1.slice(-5,-1));// it slice both +ve and negetive index and return new value
// console.log(string1.at(2));
// const join = string1.concat(" ",string3);
// console.log(join);
// console.log(string1.split('a')); //It returns an array of spliting elements

// console.log(string1.substring(1,4));
// console.log(string1.slice(-1));

// console.log(string3.toWellFormed());



